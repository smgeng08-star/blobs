...
GameServer.prototype.startWithHttpServer = function(httpServer) {
    this.log.setup(this);
...
GameServer.prototype.initServerAfterListen = function() {
    // Spawn starting food
...
GameServer.prototype.initWebSocketHandlers = function() {
    this.initServerAfterListen();

    this.socketServer.on('connection', connectionEstablished.bind(this));

    this.socketServer.on('error', function err(e) {
        console.log("[Error] WebSocket error: " + (e.message || e.code));
    });

    function connectionEstablished(ws, req) {
        if (this.getHumanCount() >= 90) { // Limit to 90 real human players (bots not included)
            ws.close();
            return;
        }

        // ----- Client authenticity check code -----
        var origin = (req && req.headers && req.headers.origin) || (ws.upgradeReq && ws.upgradeReq.headers && ws.upgradeReq.headers.origin) || '';
        ...
        function close(error) {
            ...
            if (client) {
                client.fullyDisconnected = true;
                client.disconnect = 0;
                // Immediately remove player cells
                if (client.cells) {
                    for (var i = client.cells.length - 1; i >= 0; i--) {
                        var cell = client.cells[i];
                        if (cell) this.server.removeNode(cell);
                    }
                    client.cells = [];
                }
            }

            // Immediately disconnect and remove all minions of this player!
            for (var mIdx = this.server.clients.length - 1; mIdx >= 0; mIdx--) {
                var cl = this.server.clients[mIdx];
                if (cl && cl.playerTracker && cl.playerTracker.owner === client) {
                    if (cl.playerTracker.cells) {
                        for (var cIdx = cl.playerTracker.cells.length - 1; cIdx >= 0; cIdx--) {
                            this.server.removeNode(cl.playerTracker.cells[cIdx]);
                        }
                        cl.playerTracker.cells = [];
                    }
                    cl.playerTracker.fullyDisconnected = true;
                    if (typeof cl.close === 'function') {
                        cl.close();
                    }
                    this.server.clients.splice(mIdx, 1);
                }
            }

            // Immediately remove client socket from server.clients list
            this.socket.isClosed = true;
            this.socket.sendPacket = function() { return; }; // Clear function so no packets are sent
            var clIndex = this.server.clients.indexOf(this.socket);
            if (clIndex !== -1) {
                this.server.clients.splice(clIndex, 1);
            }
        }

        ws.remoteAddress = (req && req.socket && req.socket.remoteAddress) || (ws._socket && ws._socket.remoteAddress) || '127.0.0.1';
        ws.remotePort = (req && req.socket && req.socket.remotePort) || (ws._socket && ws._socket.remotePort) || 0;
...
GameServer.prototype.queueUpdate = function(ticks) {
    if (ticks < 0 || isNaN(ticks)) ticks = 0;
    this.updateLog['loop-sleep'] = ticks;
    setTimeout(this.updateLoopBind, Math.max(0, Math.round(ticks)));
}
...
GameServer.prototype.getHumanCount = function() {
    var count = 0;
    for (var i = 0; i < this.clients.length; i++) {
        var cl = this.clients[i];
        if (cl && cl.readyState === 1 && !cl.isClosed && cl.playerTracker && !cl.playerTracker.isBot && !cl.playerTracker.owner && !cl.playerTracker.fullyDisconnected) {
            count++;
        }
    }
    return count;
